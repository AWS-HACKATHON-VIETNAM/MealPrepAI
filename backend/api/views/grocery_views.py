...
def get_grocery_list(request):
    ...
    try:
        grocery_items = GroceryList.objects.filter(user=request.user).order_by(
            "-added_at"
        )
        serializer = GroceryListSerializer(grocery_items, many=True)
...
def remove_from_grocery_list(request, item_id):
    ...
    try:
        grocery_item = get_object_or_404(GroceryList, user=request.user, id=item_id)
        grocery_item.delete()
